...
#
# Qwen3.5-122B-A10B FULL SFT — 8 nodes × 8 GPU (= 64 × H800 80G).
#
# Recipe: qwen35_vl_122b_a10b_sft_config
#   default TP=2, PP=6, EP=8, LR=2e-5, GBS=36, seq=4096
#
# IMPORTANT — parallelism on 64 GPU with 32K context:
#   Use TP=2, PP=4, CP=2, EP=4, DP=4.
#   Math: world_size=64, DP=64/(TP×PP×CP)=64/(2×4×2)=4.
#   EP must ≤ DP → EP=4.
...
#
# Memory note (122B BF16 + Adam, TP=2 PP=4 CP=2 EP=4 DP=4):
...
#
# Override knobs:
#   TP=2 PP=4 CP=2 EP=4 ITERS=20 SEQ=32768 GBS=32 MBS=1 ...
...
OUTPUT_DIR="${OUTPUT_DIR:-${MEG_RUN_DIR}/qwen35_122b_full_sft_8node_seq32768}"
LOG_DIR="${LOG_DIR:-${MEG_RUN_DIR}/logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/sft_full_8node_seq32768_$(date +%Y%m%d_%H%M%S)_rank${RANK:-0}.log}"
...
# ---------------------------------------------------------------------------
# Parallelism overrides (64 GPU / 32K layout — see header for math)
# ---------------------------------------------------------------------------
TP="${TP:-2}"
PP="${PP:-4}"
CP="${CP:-2}"
...
OVERRIDES=(
    ...
    # Parallelism (64-GPU / 32K specific override)
...
cat <<EOF
============================================================
Qwen3.5-122B-A10B FULL SFT (8-node)
  Recipe       : $RECIPE
