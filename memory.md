...
| 13 | `import mamba_ssm` (or submodules) crashes inside `docker build` with `RuntimeError: 0 active drivers ([])` | Build host has NO GPU. The chain `import mamba_ssm` → `selective_scan_interface.py` → `mamba_ssm.ops.triton.layer_norm` triggers `@triton.autotune(...)` at module load, which calls `driver.active.get_benchmarker()` and dies. Our own `__init__.py` try/except can't catch this because the autotune fires inside the imported submodule itself, deeper than the wrapped import. **Fix in Dockerfile.qwen35 (already applied 2026-04-24)**: build-time sanity uses `importlib.util.find_spec` (file presence) + `importlib.metadata.distribution` (registration) + grep for the patch marker — never `import mamba_ssm`. Real GPU-side verification still happens at container start via `scripts/runtime_sanity.sh` inside a `--gpus all` container. **Cross-cutting rule (also documented in Dockerfile.qwen35 STEP 24 header):** no `import mamba_ssm*` and no `import megatron.bridge` (same hazard, chains into `emerging_optimizers`) in any `RUN` step. |
