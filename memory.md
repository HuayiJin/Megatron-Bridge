...
| 23 | 多模态序列超出 SEQ 长度进入模型导致 shape mismatch / OOM | **Root cause:** `qwen2_5_collate_fn` 调用 `processor(...)` 时**未传 `truncation=True` 和 `max_length=seq_length`**，实际截断阈值为 tokenizer 的 `model_max_length`（~128K）。vision tokens 数量由图像分辨率决定（patch_size=14，spatial_merge=2，960×540 → 672 tokens），与文本 tokens 之和可能轻易超过 SEQ，序列原样进入 PP stage-0，产生 tensor shape 不匹配或 attention 显存越界。**数据侧 Fix（2026-04-26）：** 在写入训练 JSONL 时手动控制文本长度，确保 vision_tokens + text_tokens < SEQ（见 §Vision Token Budget）。**框架侧根本修复（2026-04-29，已完成）：** `PreloadedVLMConversationProvider._build_dataset_from_base_examples` 现在向 `VLMConversationDataset` 传递 `max_length=self.seq_length`；`VLMConversationDataset._bound_collate` 通过 `inspect.signature` 检测后安全透传给支持的 collate 函数（`qwen2_5_collate_fn` / `kimi_k25_vl_collate_fn`）；`qwen2_5_collate_fn` 对有图和无图两条路径均加入 `truncation=True, max_length=max_length`。不支持 `max_length` 的其他 collate 函数（`default_collate_fn` 等）继续原路径调用，向后兼容。 |
...
_Last updated: 2026-04-29 (Pitfall #23 框架侧根本修复完成：`qwen2_5_collate_fn` 新增 `max_length` 参数，对有图/无图两条路径均加 `truncation=True, max_length=max_length`；`VLMConversationDataset` 新增 `max_length` 参数，通过 `inspect.signature` 安全透传给支持的 collate 函数；`PreloadedVLMConversationProvider._build_dataset_from_base_examples` 传递 `max_length=self.seq_length`；ruff lint 全部通过)_
