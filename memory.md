...
>
> **CP (context parallel) 专题：** 是否能开 CP、4 条硬性前提、真根因审计，
> 见 `memory_cp_qwen35vl.md`（中文）。本文 Pitfall #23 / #24 是事件级记录，
> 那份是原理级结论。
>
...
| `qwen35_vl_122b_a10b_sft_config` (12-node, 32K, CP=2) | Full SFT | 96 (12 node × 8) | TP=2 PP=6 **CP=2** EP=4 DP=4 | 2e-5 | 32 | `scripts/run_sft_qwen35_122b_12node_hade2.sh`. Requires `--step_func qwen3_vl_step` (Pitfall #24) and the CP-flag stack (per-token loss, no average_in_collective, no native fused CE). Optimizer CPU offload ON. SEQ=32768. |
| `qwen35_vl_122b_a10b_sft_config` (24-node, 96K, CP=1) | Full SFT | 192 (24 node × 8) | TP=2 PP=12 CP=1 EP=4 DP=8 | 2e-5 | 32 | `scripts/run_sft_qwen35_122b_24node_hade.sh`. CP=1 simplifies the CP-flag stack but SEQ=96K → must set `cross_entropy_fusion_impl="te"` to avoid the ~23 GB last-stage logits OOM. PP=12 → 4 layers/stage = 1 GDN group ✓. Optimizer CPU offload ON for safety; turn off if iter-1 shows > 15 GB headroom on the last PP stage. |
...
| 23 | `torch._dynamo.exc.TorchRuntimeError: Dynamo failed to run FX node ... Attempting to broadcast a dimension of length <SEQ> at -1! Mismatching argument at index 1 had torch.Size([<SEQ>]); but expected shape should be broadcastable to [<SEQ/CP>]` during first forward of SFT with CP > 1 | **Surface symptom of Pitfall #24** (wrong step_func) — fused CE compiles `logits_2d[arange_1d, masked_target_1d]` via `@jit_fuser` (`mcore/fusions/fused_cross_entropy.py`), so dynamo's fake-tensor index op aborts the moment logits length (`SEQ/CP`) and target length (`SEQ`) disagree. With the correct step_func (Pitfall #24), labels are sliced to `SEQ/CP` up-front and this error never occurs. The defensive auto-disable still lives in `src/megatron/bridge/training/config.py` (see the `if self.model.context_parallel_size > 1` block — when `cross_entropy_loss_fusion=True` and `cross_entropy_fusion_impl=='native'`, it warns and turns fusion off, so a future CP > 1 run that forgets the step_func change would silently mis-index in unfused CE rather than dynamo-abort; the recipe `qwen35_vl.py:91-92` keeps the default `True` + comment pointing at the validator). The 12-node script also sets `model.cross_entropy_loss_fusion=False` explicitly. **Rule:** never combine native fused CE (`@jit_fuser`) with CP > 1; the validator is the single enforcement point. See pre-existing recipe-level note at `recipes/qwen/qwen3.py:617`. |
| 24 | Qwen3.5-VL SFT with CP > 1 produces fused-CE dynamo broadcast error or silently-wrong loss | **Root cause:** `--step_func vlm_step` (the historical default in every `run_sft_qwen35_122b_*.sh`) does not call `get_batch_on_this_cp_rank`. It leaves CP slicing to the model's `forward`, but `Qwen3VLModel.forward` only slices labels/loss_mask inside `if self.pre_process and combined_embeddings is not None and cp_size > 1 and packed_seq_params is None` — that branch runs **only on PP rank 0**. On the last PP stage (PP-1), where the LM head + CE live, logits get sharded to `[B, SEQ/CP, V]` (via the model's own activation slicing) but labels remain at `[B, SEQ]`. CE then mis-indexes: with native fused CE this surfaces as the dynamo broadcast crash (Pitfall #23); with unfused CE it silently produces wrong loss values via implicit broadcast / out-of-range gather. The 12-node 32K script first hit this when switching from `PP=12, CP=1` to `TP=2 PP=6 CP=2 EP=4`. **Fix (2026-04-29):** (1) `scripts/run_sft_qwen35_122b_12node_hade2.sh`: switched `--step_func vlm_step` → `--step_func qwen3_vl_step` (the Qwen3-VL-aware step that calls `get_batch_on_this_cp_rank` on `forward_args` after `pack_or_pad_batch_sequences`, guaranteeing labels/loss_mask are CP-sliced on every PP stage including the last). (2) `src/megatron/bridge/training/vlm_step.py`: added a hard `AssertionError` at the top of `get_batch` if `cp_size > 1`, pointing users at `qwen3_vl_step` / `llava_step`. This stops any other Qwen3-VL run from silently re-creating the same wrong-loss bug. (3) Kept the fused-CE auto-disable from Pitfall #23 as a second line of defence. **Rule:** for any Qwen3-VL / Qwen3.5-VL training with CP > 1 use `--step_func qwen3_vl_step`; for LLaVA use `--step_func llava_step`. The generic `vlm_step` is for CP=1 only. The other `run_sft_qwen35_122b_*.sh` scripts (4/6/8/16 nodes) currently use `vlm_step` and are safe **only** while they keep CP=1; switching them to CP > 1 requires the same step_func change. |
...
_Last updated: 2026-04-29 (Pitfall #24 vlm_step does not slice labels for CP > 1 — switched 12-node script to `qwen3_vl_step`, added hard assertion in `vlm_step.get_batch` so other scripts fail fast; Pitfall #23 reframed as the fused-CE surface symptom of #24, fused-CE auto-disable in `training/config.py` kept as second line of defence; corrected stale "DP=8" comment in 12-node optimizer-offload note. 2026-04-26: Pitfall #16 uv sync --inexact; Pitfall #17 EP>DP invalid on 32GPU; Pitfall #18 install_runtime_deps on all nodes; Pitfall #19 MTP last-stage OOM; Pitfall #20 TRITON_CACHE_DIR NAS race → FileNotFoundError; Pitfall #21 te_distributed_compat.pth venv sys.path ordering → ModuleNotFoundError; Pitfall #22 FLA Triton autotuner OOM in chunk_gated_delta_rule_bwd → FLA_AUTOTUNE=0; MTP memory note added; Standard Recipes EP corrected to EP=4; Stage D deferred, Stage E in progress)_
